...
var TicTacToe = Game.extend({

  init: function(cb) {
    var self = this
    this.container = $("<div>").addClass("tictactoe").attr("id","tictactoe").appendTo($("#tab-game"))
 ...
 */
    ...
    set = function () {
        ...
        $(this).html(turn);
        $(this).attr("class",turn=="X"?"cross":"circle")
        console.log($(this));
        moves += 1;
        score[turn] += $(this)[0].indicator;
        console.log(score[turn]);
        if (win(score[turn])) {
          self.prompt(turn + " wins!", {}, function () {
            self.start();
          })
        } else if (moves === SIZE * SIZE) {
          self.prompt("Cat\u2019s game!", {}, function () {
            self.start();
          })
        } else {
            turn = turn === "X" ? "O" : "X";
