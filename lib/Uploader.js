...
var Uploader = Base.extend({
	...
	getPath: function(uuid, ext) {
		var self = this
		return Path.join(self.opts.uploadDir, uuid + "." + ext)
	},
	/**
	...
	copy: function(params, cb) {
		...
		self.checkPath(path, function(err, fi) {
			...
			// TODO there is a bug - sometimes null UUID is passed as string...
			// only under Windows (under Linux it is fine)
			...
			self.ensureRemoved(params.uuid, function(err) {
				// console.log("Uploader.copy: ensureRemoved", err)
				if(err) {
					return cb(err)
				} 
				var size = fi.size
				var ext = getExtension(path).toLowerCase()
				var newPath = self.getPath(uuid, ext)
