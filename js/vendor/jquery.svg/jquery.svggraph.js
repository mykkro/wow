   ...
   Access through svg.graph. */
    function SVGGraph(wrapper) {
        ...
        this._chartFormat = {
            fill: 'none',
            stroke: 'black'
        }; // The formatting for the chart area
        ...
        this._chartCont = this._wrapper.svg(0, 0, 0, 0, {
            class_: 'svg-graph'
    ...
    $.extend(SVGGraph.prototype, {
	   ...
	   @return  (element) the background group element */
        _drawChartBackground: function(noXGrid, noYGrid) {
            var bg = this._wrapper.group(this._chartCont, {
                class_: 'background'
            });
        ...
        _drawAxes: function(noX) {
            ...
            if (this.yAxis) {
                if (this.yAxis._title) {
                    this._wrapper.text(this._chartCont, 0, 0, this.yAxis._title, $.extend({
                        textAnchor: 'middle',
            ...
            if (this.y2Axis) {
                if (this.y2Axis._title) {
                    this._wrapper.text(this._chartCont, 0, 0, this.y2Axis._title, $.extend({
                        textAnchor: 'middle',
	   ...
	   @param  y2    (number) ending y-coodinate for the axis */
        _drawAxis: function(axis, id, x1, y1, x2, y2) {
            var horiz = (y1 == y2);
            var gl = this._wrapper.group(this._chartCont, $.extend({
                class_: id
            }, axis._lineFormat));
            var gt = this._wrapper.group(this._chartCont, $.extend({
                class_: id + 'Labels',
            ...
            if (axis._ticks.major) {
                ...
                while (major <= axis._scale.max || minor <= axis._scale.max) {
                    ...
                    this._wrapper.line(gl, (horiz ? v : x1 + len * offsets[0]), (horiz ? y1 + len * offsets[0] : v), (horiz ? v : x1 + len * offsets[1]), (horiz ? y1 + len * offsets[1] : v));
                    if (cur == major) {
                        this._wrapper.text(gt, (horiz ? v : x1 - size), (horiz ? y1 + 2 * size : v), (axis._labels ? axis._labels[count++] : '' + cur));
                    }
	   ...
	   @return  (number[2]) the array of offset multipliers (-1..+1) */
        _getTickOffsets: function(axis, bottomRight) {
            return [(axis._ticks.position == (bottomRight ? 'in' : 'out') ||
                axis._ticks.position == 'both' ? -1 : 0), (axis._ticks.position == (bottomRight ? 'out' : 'in') ||
	   ...
	   @return  (SVGGraphAxis) percentage axis */
        ...
        _drawLegend: function() {
            ...
            var g = this._wrapper.group(this._chartCont, {
                class_: 'legend'
            });
   ...
   @return  (SVGGraphLegend) the new legend object */
    function SVGGraphLegend(graph, bgSettings, textSettings) {
        ...
        this._bgSettings = bgSettings || {
            stroke: 'gray'
        }; // Additional formatting settings for the legend background
    ...
    $.extend(SVGColumnChart.prototype, {
	   ...
	   @param  graph  (object) the SVGGraph object */
        drawGraph: function(graph) {
            ...
            this._chart = graph._wrapper.group(graph._chartCont, {
                class_: 'chart'
            });
            for (var i = 0; i < numSer; i++) {
        ...
        _drawXAxis: function(graph, numSer, numVal, barWidth, barGap, dims, xScale) {
            ...
            var gl = graph._wrapper.group(graph._chartCont, $.extend({
                class_: 'xAxis'
            }, axis._lineFormat));
            ...
            if (axis._ticks.major) {
                ...
                for (var i = 0; i < numVal; i++) {
                    ...
                    graph._wrapper.text(gt, x, dims[graph.Y] + dims[graph.H] + 2 * axis._ticks.size, (axis._labels ? axis._labels[i] : '' + i));
                }
            }
        }
    });
    ...
    $.extend(SVGRowChart.prototype, {
	   ...
	   @param  graph  (object) the SVGGraph object */
        ...
        _drawAxes: function(graph, numSer, numVal, barWidth, barGap, dims, yScale) {
            ...
            var axis = graph.xAxis;
            if (axis._title) {
                graph._wrapper.text(graph._chartCont, 0, 0, axis._title, $.extend({
                    textAnchor: 'middle',
            ...
            if (axis._ticks.major) {
                ...
                for (var i = 0; i < numVal; i++) {
                    ...
                    graph._wrapper.text(gt, dims[graph.X] - axis._ticks.size, y, (axis._labels ? axis._labels[i] : '' + i));
                }
            }
        }
    });
        ...
        _drawSeries: function(graph, dims) {
            ...
            for (var i = 0; i < numVal; i++) {
                ...
                for (var j = 0; j < numSer; j++) {
                    ...
                    if (i == 0) {
                        gl[j] = graph._wrapper.group(this._chart, $.extend({
                            class_: 'series' + j,
                            fill: series._fill,
                            stroke: series._stroke,
                            strokeWidth: series._strokeWidth
                    ...
                    var p = graph._wrapper.path(gl[j], path.reset().move(x, y).line(x + radius * Math.cos(start), y + radius * Math.sin(start)).arc(radius, radius, 0, (end - start < Math.PI ? 0 : 1), 1,
