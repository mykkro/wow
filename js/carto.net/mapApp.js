...
function mapApp(adjustVBonWindowResize,resizeCallbackFunction) {
	...
	if (!svgsvg.getScreenCTM) {
		...
		//this part is only required for viewers not supporting svgsvg.getScreenCTM() (e.g. ASV3)
		svgsvg.addEventListener("SVGScroll",this,false);
		svgsvg.addEventListener("SVGZoom",this,false);
	}
	//add SVGResize event, note that because FF does not yet support the SVGResize event, there is a workaround
 	try {
		...
		svgsvg.addEventListener("SVGResize",this,false);
	}
...
//set viewBox of svgsvg to innerWidth and innerHeight
mapApp.prototype.adjustViewBox = function() {
	svgsvg.setAttributeNS(null,"viewBox","0 0 "+this.innerWidth+" "+this.innerHeight);
}
...
mapApp.prototype.calcInvCoord = function(svgPoint) {
	if (!svgsvg.getScreenCTM) {
		var matrix = getTransformToRootElement(svgsvg);
	}
	else {
		var matrix = svgsvg.getScreenCTM();
	}
...
ViewBox.prototype.getTM = function() {
    var svgRoot      = svgsvg;
    var matrix       = svgsvg.createSVGMatrix();
